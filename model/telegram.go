...
func StartBot(token string, db *gorm.DB, TelegramId int, white string) {
	...
	b.Handle(tele.OnText, func(c tele.Context) error {
		...
		isE, _ := redis.Rdb.HExists("StartBot_"+token, "switch").Result()
		if isE == true {
			pp, _ := redis.Rdb.HGet("StartBot_"+token, "switch").Result()
			if pp == "1" {
				go MassTexting(token, TelegramId, db, c.Text())
				redis.Rdb.HSet("StartBot_"+token, "switch", "2") //1说明可以发送
			}
			redis.Rdb.HSet("StartBot_"+token, "switch", "2") //1说明可以发送
			return nil
		}
	...
	b.Handle(tele.OnPhoto, func(c tele.Context) error {
		...
		if isE == true {
			pp, _ := redis.Rdb.HGet("StartBot_"+token, "switch").Result()
			if pp == "1" {
				go MassPhotos(token, TelegramId, db, c.Text(), photo.FileID)
				redis.Rdb.HSet("StartBot_"+token, "switch", "2") //1说明可以发送
			}
			redis.Rdb.HSet("StartBot_"+token, "switch", "2") //1说明可以发送
			return nil
	...
	b.Handle(tele.OnAddedToGroup, func(context tele.Context) error {
		...
		g := Group{IdGroup: data.ID, UsernameGroup: data.Username, TypeGroup: string(data.Type), TelegramId: TelegramId}
		g.Add(db)
		return nil
	})
	b.Start()
